...
const sanitize = stats => {
  return !_.isUndefined(stats) && !_.isUndefined(stats.id);
};
...
const authorize = (proof, stats) => {
  ...
  if (
    ...
    if (
      !(addressHash.toLowerCase() === proof.address.substr(2).toLowerCase())
    ) {
      console.error(
        "API",
        "SIG",
        "Address hash did not match",
        addressHash,
...
api.on("connection", spark => {
  ...
  spark.on("hello", data => {
    ...
    if (banned.indexOf(spark.address.ip) >= 0 || !authorize(proof, stats)) {
  ...
  spark.on("block", data => {
    const { stats, proof } = data;
    if (sanitize(stats) && !_.isUndefined(stats.block)) {
      stats.id = proof.address;
  ...
  spark.on("pending", data => {
    const { stats, proof } = data;
    if (sanitize(stats) && !_.isUndefined(stats.stats)) {
      stats.id = proof.address;
