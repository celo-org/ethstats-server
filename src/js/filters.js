...
angular
  ...
  .filter("stakingClass", function() {
    return function(validatorData) {
      if (validatorData) {
        if (validatorData.elected) return "text-success";
        if (validatorData.registered) return "text-warning";
      }
      return "text-info";
    };
  })
  .filter("hashrateFilter", [
    "$sce",
    "$filter",
    function($sce, filter) {
      return function(hashes, isMining) {
        if (!isMining) return $sce.trustAsHtml('<i class="icon-cancel"></i>');
        ...
        return $sce.trustAsHtml(
          '<span class="small">' +
            filter("number")(result.toFixed(1)) +
  ...
  .filter("stakingString", function() {
    return function(validatorData) {
      if (validatorData) {
        if (validatorData.elected) return "Elected";
        if (validatorData.registered) return "Registered";
      }
      return "Full node";
    };
  })
  ...
  .filter("minerHistoryFilter", [
    "$sce",
    "$filter",
    function($sce, filter) {
      return function(miners, miner) {
        if (typeof miner === "undefined") return "?";

        var htmlStr = "";
        for (var x = miners.length - 1; x >= 0; x--) {
          var isMinerClass =
            miners[x].miner === miner.miner ? "bg-info" : "border-gray";
          htmlStr =
            htmlStr + '<span class="block ' + isMinerClass + '"></span>';
        }
